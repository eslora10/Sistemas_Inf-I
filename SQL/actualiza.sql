...
(SELECT movieid
    FROM
        (SELECT movieid, language, COUNT(*) as c FROM imdb_movielanguages group by movieid, language) AS count_lan
WHERE C>1) AND extrainformation='';
...
INSERT INTO aux(movieid, languageid, extrainformation)
SELECT movieid, languageid, extrainformation
FROM languages, imdb_movielanguages
...
SELECT orderid, prod_id, price, sum(quantity) AS quantity_2
FROM orderdetail
